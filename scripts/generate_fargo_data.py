...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ninterm", type=int, default=1)
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.ninterm < 1:
        raise ValueError("--ninterm must be positive")
...
def write_par_file(path: Path, output_dir: Path, planet_cfg: Path, case: FargoCase, args: argparse.Namespace) -> dict:
    ...
    ntot = args.ninterm * (args.frames - 1)
    dt = case_total_time / ntot
    path.write_text(
        f"""Setup               fargo_nu
...
"""
    ...
    return {"dt": dt, "ninterm": args.ninterm, "ntot": ntot, "total_time": case_total_time}
