...
def read_case(output_dir: Path, nx: int, ny: int, dt: float, ninterm: int) -> dict[str, np.ndarray]:
    ...
    for frame in frames:
        sigma.append(read_field(output_dir / f"gasdens{frame}.dat"))
        vx.append(read_field(output_dir / f"gasvx{frame}.dat"))
        vy.append(read_field(output_dir / f"gasvy{frame}.dat"))
